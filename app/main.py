...
"""
HMI Industrial - Bobinadora ESP32  v5.3
Modo manual, JOG compacto, PROX PARADA, posicion acumulada
"""
...
from recipe_manager import (validate_recipe, save_recipe, load_recipe,
...
CONFIG_FILE = "bobinadora_config.json"
...
class App(ctk.CTk):
    def __init__(self):
        ...
        # Los callbacks usan after() para ejecutar en hilo
        # principal de tkinter — evita AttributeError
        self.serial = SerialManager(
            on_message=lambda msg: self.after(
                0, lambda m=msg: self.on_serial_message(m)),
            on_status_change=lambda ok, info: self.after(
                0, lambda o=ok, i=info:
                self.on_connection_change(o, i)))
    ...
    def _build_header(self):
        hdr = ctk.CTkFrame(self, fg_color=BG_PANEL,
                           height=68, corner_radius=0)
        hdr.pack(fill="x")
        hdr.pack_propagate(False)
        ctk.CTkLabel(hdr, text="⚙  BOBINADORA HMI",
    ...
    def _build_sidebar(self, parent):
        sb = ctk.CTkFrame(parent, fg_color=BG_PANEL,
                          width=270, corner_radius=8)
        sb.grid(row=0, column=0, sticky="nsew")
        sb.pack_propagate(False)

        ctk.CTkLabel(sb, text="CONEXIÓN SERIAL",
                     font=ctk.CTkFont(*F_SMALL, "bold"),
                     text_color=TEXT_SECONDARY
                     ).pack(pady=(18, 4), padx=15, anchor="w")

        self.port_var = tk.StringVar(
            value=self.cfg.get("puerto", ""))
        self.port_combo = ctk.CTkComboBox(
            sb, variable=self.port_var, width=240,
        ...
        ctk.CTkButton(sb, text="⟳  ACTUALIZAR",
        ...
        self.btn_connect.pack(
            padx=15, pady=(4, 12), fill="x")

        ctk.CTkFrame(sb, height=1,
                     fg_color=BORDER_COLOR
                     ).pack(fill="x", padx=15)

        ctk.CTkLabel(sb, text="ESTADO ESP32",
                     font=ctk.CTkFont(*F_SMALL, "bold"),
                     text_color=TEXT_SECONDARY
                     ).pack(pady=(10, 4), padx=15, anchor="w")

        self._ind(sb, "ESTADO",
                  self.esp_estado,  ACCENT_BLUE)
        self._ind(sb, "RECETA",
                  self.esp_rec,     ACCENT_YELLOW)
        self._ind(sb, "SECCIÓN",
                  self.esp_sec,     ACCENT_PURPLE)
        self._ind(sb, "TIPO",
                  self.esp_tsec,    ACCENT_ORANGE)
        self._ind(sb, "CAPA",
                  self.esp_capa,    ACCENT_YELLOW)
        self._ind(sb, "TOTAL C.",
                  self.esp_tcap,    TEXT_SECONDARY)
        self._ind(sb, "PROX PAR",
                  self.esp_meta,    ACCENT_GREEN)
        self._ind(sb, "VUELTAS",
                  self.esp_vueltas, ACCENT_GREEN)
        self._ind(sb, "RPM",
                  self.esp_rpm,     ACCENT_ORANGE)
        self._ind(sb, "POSICIÓN",
                  self.esp_pos,     ACCENT_BLUE)

        ctk.CTkFrame(sb, height=1,
                     fg_color=BORDER_COLOR
                     ).pack(fill="x", padx=15, pady=6)

        fv = ctk.CTkFrame(sb, fg_color=BG_CARD,
                          corner_radius=6)
        fv.pack(padx=15, fill="x")
        ctk.CTkLabel(fv, textvariable=self.esp_freno,
                     font=ctk.CTkFont(*F_SMALL, "bold"),
                     text_color=TEXT_SECONDARY
                     ).pack(side="left", padx=10, pady=8)
        ctk.CTkLabel(fv, textvariable=self.esp_variador,
                     font=ctk.CTkFont(*F_SMALL, "bold"),
                     text_color=TEXT_SECONDARY
    ...
    def _build_control_tab(self):
        ...
        ctk.CTkLabel(tab, text="PANEL DE CONTROL",
                     font=ctk.CTkFont(*F_TITLE),
                     text_color=TEXT_PRIMARY
        ...
        mf.grid(row=1, column=0, columnspan=6,
                sticky="ew", padx=20)
        mf.columnconfigure((0, 1, 2, 3, 4, 5), weight=1)

        self._metric(mf, "VUELTAS",
                     self.esp_vueltas, ACCENT_GREEN,  0)
        self._metric(mf, "PROX PARADA",
                     self.esp_meta,    ACCENT_YELLOW, 1)
        self._metric(mf, "CAPA",
                     self.esp_capa,    ACCENT_ORANGE, 2)
        self._metric(mf, "RPM",
                     self.esp_rpm,     ACCENT_BLUE,   3)
        self._metric(mf, "SECCIÓN",
                     self.esp_sec,     ACCENT_PURPLE, 4)
        self._metric(mf, "TIPO",
                     self.esp_tsec,    ACCENT_ORANGE, 5)
        ...
        self.alert_label = ctk.CTkLabel(
            self.alert_frame,
            text="Sistema listo — Conecte el ESP32",
        ...
        self._ctrl_btn(bf, "▶  START",
                       ACCENT_GREEN,  "#00CC6A",
                       BG_DARK,       self._cmd_start,  0)
        self._ctrl_btn(bf, "■  STOP",
                       ACCENT_RED,    "#CC2222",
                       TEXT_PRIMARY,  self._cmd_stop,   1)
        self._ctrl_btn(bf, "↺  RESET",
                       ACCENT_YELLOW, "#CC9200",
                       BG_DARK,       self._cmd_reset,  2)
        self._ctrl_btn(bf, "⌂  HOMING",
                       ACCENT_BLUE,   "#4080CC",
                       TEXT_PRIMARY,  self._cmd_homing, 3)

        # ── Modo Manual ───────────────────────────────────────
        mf2 = ctk.CTkFrame(tab, fg_color=BG_CARD,
                            corner_radius=8)
        mf2.grid(row=4, column=0, columnspan=6,
                 sticky="ew", padx=20, pady=(0, 6))

        ctk.CTkLabel(mf2,
                     text="MODO MANUAL — sin receta, "
                          "motor libre con pedal",
                     font=ctk.CTkFont(*F_SMALL),
                     text_color=TEXT_SECONDARY
                     ).pack(side="left", padx=16, pady=12)

        self.btn_manual = ctk.CTkButton(
            mf2, text="⚙  ACTIVAR MODO MANUAL",
            command=self._cmd_manual_toggle,
            fg_color=ACCENT_ORANGE,
            hover_color="#CC6633",
            text_color=BG_DARK, height=44, width=260,
            font=ctk.CTkFont(*F_BODY_B))
        self.btn_manual.pack(
            side="right", padx=16, pady=10)

        # ── Panel JOG ─────────────────────────────────────────
        jf = ctk.CTkFrame(tab, fg_color=BG_CARD,
                          corner_radius=10)
        jf.grid(row=5, column=0, columnspan=6,
                sticky="ew", padx=20, pady=(0, 8))

        jog_title = ctk.CTkFrame(
            jf, fg_color="transparent")
        ...
        for mm in [0.1, 0.5, 1.0, 2.0,
            ...
            btn = ctk.CTkButton(
                ...
                corner_radius=6,
                command=lambda m=mm:
                self._set_jog_paso(m))
            btn.pack(side="left", padx=4, pady=8)
            self.jog_paso_btns[mm] = btn

        ctk.CTkLabel(paso_frame, text="  Manual:",
                     font=ctk.CTkFont(*F_SMALL),
                     text_color=TEXT_SECONDARY
        ...
        # Ejecutar receta
        qf = ctk.CTkFrame(tab, fg_color=BG_CARD,
                          corner_radius=8)
        qf.grid(row=6, column=0, columnspan=6,
                sticky="ew", padx=20, pady=(0, 14))

        ctk.CTkLabel(qf, text="EJECUTAR RECETA",
                     font=ctk.CTkFont(*F_HEAD),
                     text_color=TEXT_SECONDARY
                     ).pack(pady=(12, 6))
        ...
        self.run_combo = ctk.CTkComboBox(
            qr, variable=self.run_recipe_var,
            fg_color=BG_INPUT,
            border_color=BORDER_COLOR,
            button_color=BG_CARD,
            dropdown_fg_color=BG_INPUT,
            text_color=TEXT_PRIMARY,
            font=ctk.CTkFont(*F_BODY), width=360)
        self.run_combo.pack(side="left", padx=(0, 12))

        ctk.CTkButton(
            qr, text="⚡  CARGAR Y EJECUTAR",
    ...
    def _metric(self, parent, label, var, color, col):
        c = ctk.CTkFrame(parent, fg_color=BG_CARD,
                         corner_radius=10)
    ...
    def _ctrl_btn(self, parent, text,
                  color, hover, tc, cmd, col):
        ctk.CTkButton(
            parent, text=text, command=cmd,
            fg_color=color, hover_color=hover,
            text_color=tc, height=80, width=190,
    ...
    def _build_recipes_tab(self):
        ...
        ctk.CTkLabel(tab, text="GESTIÓN DE RECETAS",
                     font=ctk.CTkFont(*F_TITLE),
                     text_color=TEXT_PRIMARY
                     ).grid(row=0, column=0,
                            columnspan=2, pady=(15, 10))

        left = ctk.CTkFrame(tab, fg_color=BG_CARD,
                            corner_radius=8)
        ...
        ctk.CTkLabel(left, text="RECETAS LOCALES (JSON)",
                     font=ctk.CTkFont(*F_HEAD),
                     text_color=TEXT_SECONDARY
                     ).grid(row=0, column=0,
                            pady=(14, 6), padx=15,
        ...
        self.recipe_list_frame.grid(
            row=1, column=0, sticky="nsew",
            padx=5, pady=5)
        self.recipe_list_frame.columnconfigure(0, weight=1)

        br = ctk.CTkFrame(left, fg_color="transparent")
        br.grid(row=2, column=0, pady=10,
                padx=10, sticky="ew")

        ctk.CTkButton(br, text="+ NUEVA",
                      command=self._open_recipe_form,
                      fg_color=ACCENT_GREEN,
                      hover_color="#00CC6A",
                      text_color=BG_DARK, height=46,
                      font=ctk.CTkFont(*F_BODY_B)
                      ).pack(side="left", expand=True,
                             fill="x", padx=3)

        ctk.CTkButton(br, text="🗑 BORRAR",
                      command=self._delete_selected_recipe,
                      fg_color=ACCENT_RED,
                      hover_color="#CC2222",
                      text_color=TEXT_PRIMARY, height=46,
                      font=ctk.CTkFont(*F_BODY_B)
                      ).pack(side="left", expand=True,
                             fill="x", padx=3)

        right = ctk.CTkFrame(tab, fg_color=BG_CARD,
                             corner_radius=8)
        ...
        ctk.CTkLabel(right, text="DETALLE DE RECETA",
                     font=ctk.CTkFont(*F_HEAD),
                     text_color=TEXT_SECONDARY
                     ).pack(pady=(14, 6), padx=15,
                            anchor="w")

        self.recipe_detail = ctk.CTkTextbox(
            right, fg_color=BG_INPUT,
            text_color=ACCENT_GREEN,
            font=ctk.CTkFont(*F_BODY),
            border_color=BORDER_COLOR, border_width=1,
            state="disabled")
        self.recipe_detail.pack(fill="both", expand=True,
                                padx=10, pady=(0, 5))

        br2 = ctk.CTkFrame(right, fg_color="transparent")
        br2.pack(pady=(0, 12), padx=10, fill="x")

        ctk.CTkButton(br2, text="📤 ENVIAR AL ESP32",
    ...
    def _build_position_tab(self):
        ...
        ctk.CTkLabel(
            tab,
            text="Introduce la vuelta acumulada desde "
                 "el inicio de la sección. "
                 "La capa se detecta automáticamente.",
            font=ctk.CTkFont(*F_BODY),
            text_color=TEXT_SECONDARY, wraplength=900
        ).pack(pady=(0, 12))
        ...
        self.pos_recipe_combo = ctk.CTkComboBox(
            card, variable=self.pos_recipe_var,
            fg_color=BG_INPUT,
            border_color=BORDER_COLOR,
            button_color=BG_CARD,
            dropdown_fg_color=BG_INPUT,
            text_color=TEXT_PRIMARY,
            font=ctk.CTkFont(*F_BODY), width=320,
            command=self._on_pos_recipe_change)
        self.pos_recipe_combo.grid(
            row=0, column=1, padx=10,
            pady=(20, 5), sticky="ew", columnspan=3)

        # Sección
        ctk.CTkLabel(card, text="Sección:",
                     font=ctk.CTkFont(*F_BODY_B),
                     text_color=TEXT_PRIMARY
        ...
        self.pos_sec_combo = ctk.CTkComboBox(
            card, variable=self.pos_sec_var,
            values=["1"],
            fg_color=BG_INPUT,
            border_color=BORDER_COLOR,
            button_color=BG_CARD,
            dropdown_fg_color=BG_INPUT,
            text_color=TEXT_PRIMARY,
            font=ctk.CTkFont(*F_BODY), width=130,
            command=lambda v: self._update_pos_info())
        self.pos_sec_combo.grid(
        ...
        # Capa detectada (solo lectura)
        ctk.CTkLabel(card, text="Capa detectada:",
                     font=ctk.CTkFont(*F_BODY_B),
                     text_color=TEXT_PRIMARY
        ...
        self.pos_capa_info.grid(
            row=2, column=2, padx=20,
            pady=10, sticky="w", columnspan=2)

        # Vuelta acumulada
        ctk.CTkLabel(card, text="Vuelta acumulada:",
                     font=ctk.CTkFont(*F_BODY_B),
                     text_color=TEXT_PRIMARY
        ...
        vb.grid(row=3, column=2, padx=5, pady=10,
        ...
        for delta, label, color in [
            ...
            ctk.CTkButton(
                vb, text=label, width=74, height=42,
                font=ctk.CTkFont(
                    "Consolas", 13, "bold"),
                fg_color=BG_INPUT,
                hover_color=BORDER_COLOR,
                ...
                self._inc_pos("vuelta", d)
            ).pack(side="left", padx=3)

        ctk.CTkLabel(
            card,
            text="Vueltas acumuladas desde inicio de "
                 "sección — ej: S4 C5 = 461v",
            font=ctk.CTkFont(*F_SMALL),
            text_color=TEXT_SECONDARY
        ...
        ctk.CTkButton(
            ...
            fg_color=ACCENT_GREEN,
            hover_color="#00CC6A",
            text_color=BG_DARK, height=62, width=500,
            font=ctk.CTkFont("Consolas", 18, "bold")
        ).pack(pady=18)
    ...
    def _build_config_tab(self):
        ...
        params = [
            ("Espesor del alambre (mm)",
             "espesor_mm",       "1.0",
             "0.1 – 25.0", "float"),
            ("Retardo freno (segundos)",
             "retardo_freno",    "1.5",
             "0.0 – 10.0", "float"),
            ("Lógica freno  (1=NO, 0=NC)",
             "freno_no",         "1",
             "0 o 1",      "bool"),
        ...
        ctk.CTkButton(
            br, text="📤  GUARDAR Y ENVIAR AL ESP32",
        ...
        ctk.CTkLabel(
            card,
            text="Los parámetros se guardan en "
                 "bobinadora_config.json y se envían "
                 "al ESP32 automáticamente al conectar.",
            font=ctk.CTkFont(*F_SMALL),
            text_color=TEXT_SECONDARY, wraplength=700
        ).pack(pady=(0, 16))
    ...
    def _build_monitor_tab(self):
        ...
        ctk.CTkButton(hdr, text="🗑 LIMPIAR",
    ...
    def _set_jog_paso(self, mm: float):
        ...
        for m, btn in self.jog_paso_btns.items():
            if abs(m - mm) < 0.001:
                btn.configure(fg_color=ACCENT_YELLOW,
                              text_color=BG_DARK)
            else:
        ...
        try:
            ...
            for btn in self.jog_paso_btns.values():
                btn.configure(fg_color=BG_CARD,
                              text_color=TEXT_SECONDARY)
            self.jog_paso_var.set(mm)
            self.jog_paso_actual.configure(
                text=f"{mm:.2f}mm")
    ...
    def _jog_pulso(self, direction: str):
        ...
        def _t():
            ...
            self.log(f"JOG {direction} {lbl} "
                     f"({pasos}p): {resp}", "info")
            if resp and any("ERR" in x for x in resp):
                self.after(0, lambda r=resp:
                           self.jog_status.configure(
                               text=f"✗ {r[0][:20]}",
                               text_color=ACCENT_RED))
            else:
    ...
    def _jog_start(self, direction: str):
        ...
        self.jog_status.configure(
            ...
            text_color=ACCENT_BLUE)
        self.serial.send(
            "JOG:LEFT" if direction == "left"
            else "JOG:RIGHT")
        threading.Thread(
            target=self._jog_loop, daemon=True).start()

    def _jog_loop(self):
        while self._jog_active:
            cmd = ("JOG:LEFT"
                   if self._jog_direction == "left"
                   else "JOG:RIGHT")
    ...
    def _jog_stop(self):
        ...
        for btn in [self.jog_left_btn, self.jog_right_btn]:
            btn.configure(fg_color="#1C3A5C",
                          text_color=ACCENT_BLUE)
        self.jog_status.configure(
            text="◉ PARADO", text_color=TEXT_SECONDARY)
    ...
    def _leer_cfg_entries(self) -> dict:
        ...
        for key, (entry, tipo) in self.cfg_entries.items():
            try:
                v = entry.get().strip()
                if   tipo == "float": cfg[key] = float(v)
                elif tipo == "int":   cfg[key] = int(v)
                elif tipo == "bool":
    ...
    def _enviar_config_esp(self):
        ...
        if not self.connected:
            messagebox.showerror(
                "Error", "No hay conexión con el ESP32")
            return
        threading.Thread(
    ...
    def _send_config_to_esp(self):
        ...
        cmd     = (f"CONFIG:esp={esp_x10},retf={retf},"
                   f"frenoNO={freno},dirinit={dirinit},"
                   f"vpre={vpre}")
        resp    = self.serial.send(cmd)
    ...
    def _update_pos_info(self):
        ...
        try:
            ...
            # Validar vuelta acumulada
            try:
                v_actual = float(
                    self.pos_vuelta_var.get())
                if v_actual > total_sec:
                    self.pos_vuelta_var.set(
                        str(total_sec))
                    v_actual = total_sec
                if v_actual < 0:
                    self.pos_vuelta_var.set("0.0")
            ...
            # Detectar capa desde vuelta acumulada
            capa_idx = 0
            ...
            capa_num     = capa_idx + 1
            ant          = (capas[capa_idx - 1]
                            if capa_idx > 0 else 0.0)
            meta         = capas[capa_idx]
            vueltas_capa = round(meta - ant, 1)
            d = ("->" if sec["dirs"][capa_idx]
                 else "<-")

            self.pos_capa_var.set(str(capa_num))
            self.pos_capa_info.configure(
                text=f"Capa {capa_num} "
                     f"({vueltas_capa:.0f}v)  {d}\n"
    ...
    def _inc_pos(self, field, delta):
        if field == "vuelta":
            try:
                ...
                v = round(float(
                    self.pos_vuelta_var.get()) + delta, 1)
    ...
    def _apply_position(self):
        ...
        resumen = (
            f"Receta   : {rec_name}\n"
            f"Sección  : {sec_num} — "
            f"{sec.get('nombre','')} [{sec_tipo}]\n"
            f"─────────────────────────────\n"
            f"Vuelta acum. : {vuelta_acum:.1f}v "
            f"(de {total_sec:.1f}v totales)\n"
            f"Capa detect. : {capa_num}/{len(capas)} "
            f"({vuelta_en_capa:.1f}v dentro de capa)\n"
            f"Encoder      : {pulsos} pulsos"
            f"{prox_der}\n"
            f"─────────────────────────────\n"
            f"El ESP32 iniciará en S{sec_num} "
            f"con encoder={pulsos}."
        )

        if not messagebox.askyesno(
            "Confirmar inicio", resumen):
            return

        def _thread():
            self.log(
                f"=== REANUDANDO S{sec_num} "
                f"V_acum={vuelta_acum} ===", "info")

            self._send_recipe_thread(recipe)
            time.sleep(0.3)

            if pulsos > 0:
                resp = self.serial.send(
                    f"SETENC:{pulsos}")
                self.log(
                    f"SETENC {pulsos} pulsos "
                    f"({vuelta_acum:.1f}v): {resp}",
                    "info")
                time.sleep(0.15)
            else:
                self.log(
                    "Vuelta 0 — encoder en 0", "info")
    ...
    def _recipe_summary(self, recipe):
        lines = [
            ...
            f"ESPESOR  : "
            f"{recipe.get('espesorX10',10)/10:.1f} mm",
            f"SECCIONES: "
            f"{len(recipe.get('secciones',[]))}",
            "",
        ]
        for i, sec in enumerate(
                recipe.get("secciones", [])):
            tipo = sec.get("tipo", "BOB")
            nom  = sec.get("nombre", "")
            ic   = "⚙" if tipo == "BOB" else "📄"
    ...
    def _send_recipe_thread(self, recipe):
        ...
        self.log(f"  NEW: {r}")
        r = self.serial.send(
            f"ESP:{recipe.get('espesorX10',10)}")
        self.log(f"  ESP: {r}")
        ...
        if r and any("ERR" in x for x in r):
            self.log(
                f"✗ ESP32 rechazó la receta: {r}",
                "error")
        else:
            self.log(
                f"✓ '{nombre}' cargada en ESP32", "ok")
    ...
    def _on_recipe_saved(self, recipe):
        ...
        if not resultado:
            messagebox.showerror(
                ...
                "'recetas/' junto a main.py.")
            return

        self._load_recipe_list()
        self.log(
            f"✓ Receta '{recipe['nombre']}' guardada",
            "ok")

        if self.connected and messagebox.askyesno(
            "Enviar al ESP32",
            "¿Enviar la receta al ESP32 ahora?"
        ):
            threading.Thread(
                target=self._send_recipe_thread,
    ...
    def log(self, msg, tag="normal"):
        ...
        def _ins():
            ...
            self.monitor_box._textbox.insert(
                "end", txt, tag)
    ...
    def on_serial_message(self, msg):
        ...
        tag = "normal"
        if   "PAUSA:CAPA"    in msg: tag = "pause"
        elif "PAUSA:DER"     in msg: tag = "pause"
        elif "PAUSA:BARRERA" in msg: tag = "barrera"
        elif "TERMINADA"     in msg: tag = "ok"
        elif "ERR"           in msg: tag = "error"
        elif "OK"            in msg: tag = "ok"
        elif "SECCION"       in msg: tag = "info"
        elif "MANUAL"        in msg: tag = "manual"
        self.log(msg, tag)

        if ("PAUSA:DER" in msg or
                "PAUSA:BARRERA" in msg):
            parts = msg.split(":")
            alert = ""
            for i, p in enumerate(parts):
                if p == "MSG" and i+1 < len(parts):
                    alert = parts[i+1]; break
            if not alert:
                for i, p in enumerate(parts):
                    if (p in ("DER", "BARRERA") and
                            i+1 < len(parts)):
                        alert = f"⚡ {parts[i+1]}"; break
            self.after(0, lambda a=alert:
                       self._show_alert(a, ACCENT_YELLOW))

        elif ("PAUSA:CAPA" in msg or
              "PAUSA:CAPA_BARRERA" in msg):
            parts = msg.split(":")
            cn = parts[2] if len(parts) > 2 else "?"
            self.after(0, lambda c=cn:
                       self._show_alert(
                           f"FIN CAPA {c} "
                           f"— Presione ▶ START",
                           ACCENT_YELLOW))
            ...
            nxt = ""
            for i, p in enumerate(parts):
                if (p == "NEXT_NOMBRE" and
                        i+1 < len(parts)):
                    nxt = parts[i+1]; break
            self.after(0, lambda n=nxt:
                       self._show_alert(
                           f"FIN SECCIÓN → "
                           f"Siguiente: {n} "
                           f"— Presione ▶ START",
    ...
    def _parse_status(self, msg):
        ...
        estado_str = ESTADOS.get(
            estado_num, f"EST_{estado_num}")
        self.after(0, lambda s=estado_str:
                   self.esp_estado.set(s))
        ...
        upd(self.esp_pos,     "POS",
            lambda v: f"{v}cm")

        pos_val = d.get("POS", "")
        if pos_val and hasattr(self, 'jog_pos_label'):
            self.after(0, lambda v=pos_val:
                       self.jog_pos_label.configure(
                           text=f"Pos: {v}cm"))

        capa  = d.get("CAPA",  "--")
        dcapa = d.get("DCAPA", "0")
        cdisp = (dcapa if dcapa not in ("0", "")
                 else capa)
        self.after(0,
                   lambda v=cdisp: self.esp_capa.set(v))

        freno = d.get("FRENO", "0")
        var   = d.get("VAR",   "0")
        f_txt = ("🔒 FRENO" if freno == "1"
                 else "🔓 libre")
        v_txt = ("⚡ MOTOR" if var == "1"
                 else "⏹ parado")
        self.after(0,
                   lambda f=f_txt: self.esp_freno.set(f))
        self.after(0,
                   lambda v=v_txt:
                   self.esp_variador.set(v))

        es_manual = (estado_num == "13")
        self.after(0, lambda m=es_manual:
                   self._sync_manual_btn(m))

        alertas = {
            "0":  ("Sistema listo — Cargue una receta",
                   TEXT_SECONDARY),
            "1":  ("● BOBINANDO — Pise pedal para parar",
                   ACCENT_GREEN),
            "2":  ("◐ PREFRENO — Reduciendo velocidad...",
                   ACCENT_YELLOW),
            "3":  ("⏸ FIN DE CAPA — Presione ▶ START",
                   ACCENT_YELLOW),
            "4":  ("▶ DESBLOQUEADO — Pise el PEDAL",
                   ACCENT_BLUE),
            "5":  ("⚡ PAUSA DER — Presione ▶ START",
                   ACCENT_RED),
            "6":  ("▶ DER DESBLOQUEADA — Pise el PEDAL",
                   ACCENT_BLUE),
            "7":  ("⏭ FIN SECCIÓN — Presione ▶ START",
                   ACCENT_BLUE),
            "8":  ("✓ BOBINA COMPLETA — Presione ▶ START",
                   ACCENT_GREEN),
            "9":  ("🔧 JOG — Mueva el husillo",
                   ACCENT_BLUE),
            "10": ("📄 BARRERA — Pise el pedal para girar",
                   ACCENT_PURPLE),
            "11": ("📄 PAUSA BARRERA — Presione ▶ START",
                   ACCENT_PURPLE),
            "12": ("⌂ HOMING en progreso...",
                   ACCENT_ORANGE),
            "13": ("⚙ MODO MANUAL — Pise PEDAL para girar",
                   ACCENT_ORANGE),
        }
        if estado_num in alertas:
            txt, color = alertas[estado_num]
            self.after(0, lambda t=txt, c=color:
                       self._show_alert(t, c))

        rec_name = d.get("REC", "")
        if rec_name and rec_name != "ninguna":
            self.after(0, lambda n=rec_name:
                       self.run_recipe_var.set(n))
        ...
        def _upd():
            if connected:
                ...
                self.btn_connect.configure(
                    ...
                    text_color=BG_DARK)
                self.log(
                    f"Desconectado: {info}", "error")
                self._show_alert(
                    "Desconectado — Reconecte el ESP32",
                    ACCENT_RED)
                self._manual_activo = False
                self.btn_manual.configure(
                    text="⚙  ACTIVAR MODO MANUAL",
...
class RecipeForm(ctk.CTkToplevel):
    def __init__(self, parent, recipe=None,
        ...
        self.title(
            "Editor de Receta — Bobinadora HMI v5.3")
        ...
        self.sec_tipo   = [tk.StringVar(value="BOB")
                           for _ in range(8)]
        self.sec_nombre = [tk.StringVar()
                           for _ in range(8)]
        self.num_secs   = 0
    ...
    def _build(self, recipe):
        ...
        ctk.CTkLabel(
            hdr,
            text="CREAR RECETA" if not self.editing
                 else "EDITAR RECETA",
            font=ctk.CTkFont(*F_TITLE),
            text_color=ACCENT_GREEN
        ...
        ctk.CTkButton(
            footer, text="✓  GUARDAR RECETA",
            command=self._save,
            fg_color=ACCENT_GREEN,
            hover_color="#00CC6A",
            text_color=BG_DARK, height=50, width=380,
            font=ctk.CTkFont("Consolas", 16, "bold")
    ...
    def _build_basic(self, recipe):
        ...
        ctk.CTkLabel(f, text="DATOS BÁSICOS",
                     font=ctk.CTkFont(*F_HEAD),
                     text_color=TEXT_SECONDARY
    ...
    def _build_add_section_btn(self):
        self.add_sec_btn = ctk.CTkButton(
            ...
            text_color=ACCENT_BLUE, height=48,
            font=ctk.CTkFont(*F_BODY_B))
        self.add_sec_btn.pack(
            fill="x", padx=16, pady=6)

    def _add_section(self, recipe=None):
        if self.num_secs >= 8:
            messagebox.showwarning(
                "Aviso", "Máximo 8 secciones")
            return
        ...
        hdr = ctk.CTkFrame(frame, fg_color=BG_INPUT,
                           corner_radius=8)
        ...
        ctk.CTkButton(
            hdr, text="＋ AGREGAR CAPA",
            command=lambda s=sec_idx:
            self._add_layer(s),
            fg_color=ACCENT_GREEN,
            hover_color="#00CC6A",
            text_color=BG_DARK, height=38, width=180,
            font=ctk.CTkFont(*F_BODY_B)
        ).pack(side="right", padx=10, pady=10)

        tipo_row = ctk.CTkFrame(
            frame, fg_color="transparent")
        tipo_row.pack(fill="x", padx=12, pady=4)
        ctk.CTkLabel(tipo_row, text="Tipo:",
                     font=ctk.CTkFont(*F_BODY_B),
                     text_color=TEXT_PRIMARY
        ...
        ctk.CTkLabel(tipo_row, text="  Nombre:",
                     font=ctk.CTkFont(*F_BODY),
                     text_color=TEXT_SECONDARY
        ...
        nom_e = ctk.CTkEntry(
            ...
            border_color=BORDER_COLOR,
            text_color=TEXT_PRIMARY,
            font=ctk.CTkFont(*F_BODY), width=240)
        nom_e.pack(side="left", padx=6)
        if recipe and recipe.get("nombre"):
            self.sec_nombre[sec_idx].set(
                recipe["nombre"])

        col_hdr = ctk.CTkFrame(
            frame, fg_color=BG_INPUT, corner_radius=6)
    ...
    def _add_layer(self, sec_idx,
                   meta=None, direction=None):
        rows_frame = \
            self.sec_frames[sec_idx]._rows_frame
        num = len(self.sec_layers[sec_idx]) + 1
        if direction is None:
            direction = (num % 2 == 1)
        row = LayerRow(
            rows_frame, num, meta, direction,
            on_delete=lambda r, s=sec_idx:
            self._del_layer(s, r))
    ...
    def _del_layer(self, sec_idx, row):
        ...
        for i, r in enumerate(
                self.sec_layers[sec_idx]):
            r.update_num(i + 1)

    def _build_derivaciones(self, parent,
                             sec_idx, recipe=None):
        f = ctk.CTkFrame(parent, fg_color=BG_INPUT,
                         corner_radius=8)
        ...
        ctk.CTkButton(
            hdr, text="＋ AGREGAR",
            command=lambda s=sec_idx:
            self._add_derivacion(s),
            fg_color=ACCENT_YELLOW,
            hover_color="#CC9200",
            text_color=BG_DARK, height=36, width=160,
            font=ctk.CTkFont(*F_BODY_B)
        ).pack(side="right")
        der_frame = ctk.CTkFrame(
    ...
    def _add_derivacion(self, sec_idx,
                        vuelta=None,
                        etiqueta=None,
                        mensaje=None):
        der_frame = (self.sec_frames[sec_idx]
                     ._der_section._der_frame)
        row = DerivacionRow(
            der_frame, vuelta, etiqueta, mensaje,
            on_delete=lambda r, s=sec_idx:
            self._del_der(s, r))
        ...
        for sec_idx in range(self.num_secs):
            ...
            for c in range(1, len(capas)):
                if capas[c] <= capas[c - 1]:
                    messagebox.showerror(
                        "Error",
                        f"S{sec_idx+1}: Capa {c+1} "
                        f"({capas[c]}) debe ser mayor "
...
class LayerRow(ctk.CTkFrame):
    def __init__(self, parent, num, meta=None,
                 direction=True, on_delete=None):
    ...
    def _build(self, meta):
        self.num_label = ctk.CTkLabel(
            self, text=f"{self.num:2d}", width=70,
            font=ctk.CTkFont("Consolas", 16, "bold"),
            text_color=ACCENT_BLUE)
        self.num_label.pack(
    ...
    def _dir_text(self):
        return ("→  HACIA MAX" if self._direction
                else "←  HACIA MIN")

    def _dir_color(self):
        return (DIR_FWD_COLOR if self._direction
                else DIR_REV_COLOR)

    def _dir_hover(self):
        return ("#00CC6A" if self._direction
                else "#CC2222")
...
class DerivacionRow(ctk.CTkFrame):
    def __init__(self, parent, vuelta=None,
                 etiqueta=None, mensaje=None,
                 on_delete=None):
    ...
    def _build(self, vuelta, etiqueta, mensaje):
        ...
        row1.pack(fill="x")
        ctk.CTkLabel(row1, text="⚡ Vuelta:",
                     font=ctk.CTkFont(*F_BODY),
                     text_color=ACCENT_YELLOW
                     ).pack(side="left",
                            padx=(10, 4), pady=6)
        self.vuelta_entry = ctk.CTkEntry(
            row1, placeholder_text="ej: 15.5",
            fg_color=BG_CARD,
            border_color=BORDER_COLOR,
            text_color=ACCENT_YELLOW,
            font=ctk.CTkFont("Consolas", 14),
        ...
        self.vuelta_entry.pack(
            side="left", padx=6, pady=6)
        ctk.CTkLabel(row1, text="Etiqueta:",
                     font=ctk.CTkFont(*F_BODY),
                     text_color=TEXT_SECONDARY
                     ).pack(side="left", padx=(14, 4))
        self.etiq_entry = ctk.CTkEntry(
            row1, placeholder_text="ej: TAP1",
            fg_color=BG_CARD,
            border_color=BORDER_COLOR,
            text_color=TEXT_PRIMARY,
            font=ctk.CTkFont("Consolas", 14),
        ...
        self.etiq_entry.pack(
            side="left", padx=6, pady=6)
        ctk.CTkButton(
            row1, text="✕", width=42, height=36,
            fg_color="transparent",
            hover_color=ACCENT_RED,
            text_color=ACCENT_RED,
            font=ctk.CTkFont("Consolas", 15, "bold"),
        ...
        row2.pack(fill="x")
        ctk.CTkLabel(row2, text="💬 Mensaje:",
                     font=ctk.CTkFont(*F_SMALL),
                     text_color=TEXT_SECONDARY
                     ).pack(side="left",
                            padx=(10, 4), pady=(0, 8))
